...
const statusColorMap: Record<ProjectStatus, string> = {
  ...
  'confirmed': 'bg-status-confirmed/20',
...
const InstallersView = ({ projects, installers: allInstallers, days, colWidth, startDate, todayStr, onSelectProject, onDropProject }: InstallersViewProps) => {
  // Group projects by installer, include "Unassigned" row
  const groups = useMemo(() => {
    ...
    allInstallers.forEach(inst => {
      map.push({
        installer: inst,
        projects: projects.filter(p => p.assigneeId === inst.id),
      });
    });

    const unassigned = projects.filter(p => !p.assigneeId);
    if (unassigned.length > 0) {
      map.push({ installer: null, projects: unassigned });
    }

    return map;
  }, [projects, allInstallers]);
  ...
  // Calculate occupancy for each installer over visible days
    ...
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="shrink-0 border-r border-border bg-card" style={{ width: labelWidth }}>
        ...
        {groups.map((group, idx) => {
          ...
          const occupancy = inst ? getOccupancy(inst, group.projects) : 0;
          return (
            <div
