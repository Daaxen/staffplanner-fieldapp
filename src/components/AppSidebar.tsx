import { Calendar, Users, LayoutDashboard, ClipboardList, Send, FileText, Settings } from 'lucide-react';
...
const AppSidebar = ({ activeView, onViewChange }: AppSidebarProps) => {
  return (
    <aside className="w-64 bg-sidebar text-sidebar-foreground flex flex-col h-screen shrink-0">
      {/* Logo */}
      <div className="p-6 border-b border-sidebar-border">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-sidebar-ring flex items-center justify-center">
      ...
      <nav className="flex-1 p-4 space-y-1">
        <p className="text-xs font-medium text-sidebar-foreground/40 uppercase tracking-wider mb-3 px-3">Menu</p>
        {navItems.map((item) => (
          <button
            key={item.id}
